...
def convert_vitlayer(paras):
    new_para_name = ''
    if paras[0] == 'ln_1':
        new_para_name = '.'.join(['norm1'] + paras[1:])
    elif paras[0] == 'attn':
        
        new_para_name = '.'.join(['attn.attn'] + paras[1:])
    elif paras[0] == 'ln_2':
        new_para_name = '.'.join(['norm2'] + paras[1:])
    elif paras[0] == 'mlp':
        if paras[1] == 'c_fc':
    ...
    for k, v in ckpt.items():
        ...
        if key_list[0] == 'visual':
            ...
            if key_list[1] == 'class_embedding':
                ...
                new_name = '.'.join([
                    new_transform_name, 'patch_embed.proj', key_list[2]
                ])
                ...
                # if key_list[2] == 'resblocks':
                new_layer_name = 'blocks'
